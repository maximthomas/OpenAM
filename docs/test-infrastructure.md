...
| Spec | Covers |
|---|---|
| `e2e/oauth2/oauth2-test.spec.mjs` | authorize (code + PKCE S256) → `POST /oauth2/access_token` → `GET /oauth2/userinfo` with a **Bearer header**. Creates the OAuth2 service + a public client `test_client_app` (scope `profile`) via `/json/realms/root/realm-config/…` if absent. Also carries the Restlet-migration **contract locks** §5-E (`/access_token` + cache headers), §5-E2 (`/authorize`) and §5-E5 (**routing**: realm styles, path forms, unmapped verbs, the method tunnel and the request `Host` — 14 rows), all recorded against live Restlet — see `docs/migration/restlet/`. ⚠ §5-E5 owns a `node:http` raw client and a sub-realm fixture, because a `Host` header that does not match the connection, and path bytes like `//tokeninfo`, cannot be expressed through Playwright. |
