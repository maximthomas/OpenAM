...
| 5d-1 | **the flip** — `OAuth2HttpRouteProvider` (18 attachments + audit matrix) + `META-INF/services` + the web.xml mapping move; Restlet left dormant behind a one-line revert | **done 2026-08-05** — **5-E5** ✅ 2026-08-04 · **5d-1a** ✅ · **5d-1b** ✅ · **5d-1c** ✅ the flip, e2e 131 passed / 1 skipped / 0 failed, divergence rows 14–30 recorded ([spec](phase-5d-1.md) · [as-built](phase-5d-1-asbuilt.md)). Criteria 12–16 all green **and recorded** — 16 on 2026-08-06: CI run `31031060622`, 9/9 legs + `build-docker`, on the SHA the branch still carries ([record](phase-5d-1-asbuilt.md#criterion-16--ci-nine-legs--recorded-2026-08-06)). **criterion 17, the soak** ✅ 2026-08-06 ([record](phase-5d-1-asbuilt.md#criterion-17--the-soak--recorded-2026-08-06)) — 3 fresh-env full-suite runs with **zero flake**, both tripwires clear, a 90,689-request load probe clean, and the three coverage gaps closed with 21 new rows. It **found one regression the byte-diff missed** — confidential-client and resource-owner authentication broken in non-root realms on every path spelling — root-caused, fixed, and guarded; details in the record. ✅ **The fix is now in CI too** (corrected 2026-08-08 — this cell said "not yet in CI", which criterion 16 made true only for the flip commit): the fix is `d52496710e`, an ancestor of `83f465452b`, and CI run `31105613611` on that SHA is **green** — the same run the 5d-2 row below cites as 5d-2b's gate |
| 5d-2 | **the deletion** — the Restlet OAuth2 stack + `ForgeRockRest` + Guice unbinds + the core de-leak | **5d-2a ✅ done 2026-08-08** (both halves — [as-built](phase-5d-2-asbuilt.md); e2e 159 declared / 158 passed / 1 skipped, divergence rows 33–37 recorded, `grep -c "org.restlet" web.xml` → 0). **The tail was re-planned 2026-08-08 into six sub-phases** — 5d-2b-i (freeze the oracles), 5d-2b-ii (the endpoint deletion), 5d-2b-iii (the transport de-plumb), 5d-2c (openam-rest), 5d-2d-i (the `OAuthProblemException` bridge — the phase's only deliberate behaviour change), 5d-2d-ii (the sweep + exit gates). It replaces the four-way split below, which was sized against a research file that under-counted in four places: nine Restlet-legged tests not five, openam-uma carrying two real `org.restlet.Request` overloads rather than vestigial imports, `RestRealmValidator` reaching **openam-sts and openam-entitlements**, and `@ExceptionHandler` dispatch being polymorphic. All four are measured in [research §11](phase-5d-2-research.md#11). ⚠ Rows 31–32 are still owed, by **5d-2d-i**. — *superseded planning record follows:* **planned 2026-08-06, split four ways / five commits** ([spec](phase-5d-2.md) · [research](phase-5d-2-research.md)); **5d-2a reviewed 2026-08-06** and split again into **5d-2a-i** (build the CHF route dormant) + **5d-2a-ii** (flip `web.xml`, delete the Restlet classes), with four corrections recorded in [research §1 review](phase-5d-2-research.md#1-review): **5d-2a** WebFinger → CHF (pulled forward from phase 6 — it holds twelve classes hostage), **5d-2b** the openam-oauth2 stack, **5d-2c** the openam-rest layer, **5d-2d** the core de-leak + exit gates. The soak's fix **is** committed and pushed (`83f465452b` on `upstream`); its CI run `31105613611` is the gate on 5d-2b's first `git rm`. ⚠ Two of [§ Handed to 5d-2](phase-5d-1-asbuilt.md#handed-to-5d-2)'s **six** items are out of date — **trap 1** (`LoginHintHook`'s Restlet methods are three, not two — [research §2](phase-5d-2-research.md#2)) and **trap 5** (both `RestletRealmRouter`s die, rather than one surviving — [research §5](phase-5d-2-research.md#5)). Corrected in place 2026-08-08; the count said "five" |
...
#### Rows 31–37 — added by [phase 5d-2](phase-5d-2.md#divergence-rows-this-phase-adds-to-planmd)

⚠ **Different incumbent, rows 31–32 only.** Restlet stopped serving `/oauth2` at 5d-1c, so for those
two rows the "before" column is the **post-flip CHF stack**, not Restlet. Rows 33–37 are `/.well-known`,
which Restlet still served until 5d-2a-ii, so their "before" column **is** Restlet. (Corrected
2026-08-08: the note previously said "these three rows" and predated rows 35–37.) The table's rule is
unchanged: a diff matching a row is expected, a diff matching none is a regression.
...
| 37 | `GET /.well-known/` — **bare, with the trailing slash** | **404**, 102 B, `{"error":"Not Found","error_description":"The server has not found anything matching the request URI"}` — Restlet's `OAuth2StatusService` | **500**, `Content-Length: 0`, **no body and no `Content-Type`** | ⚠ **Pre-existing CHF servlet/router behaviour, not introduced by the flip.** Measured on the same container: `/oauth2/`, `/uma/` and `/json/` all answer the identical bodyless 500, while `/oauth2` (404, 53 B), `/json` (404, 75 B) and `/.well-known` (404, 53 B) answer properly — `/json` is the oldest CHF surface in the product and predates this migration, and `/oauth2/` is already [row 27](#expected-divergences-at-the-flip). Root cause is [row 27](#expected-divergences-at-the-flip)'s, measured there: commons `RouteMatchers.getRemainingRequestUri:164-170` sublists without a guard → `IllegalArgumentException: fromIndex(4) > toIndex(3)`. This row exists only because criterion 8 requires every probe to land somewhere and row 27 is scoped to `/oauth2/`; it is **the same defect on a second path**, tracked in [decisions.md](decisions.md#chf-cleanup-backlog). No stack trace reaches `catalina.out`; the exception is swallowed. **Narrowing** — a framework defect newly exposed on one more path, trading Restlet's 404 for the CHF stack's uniform bodyless 500. Measured at [5d-2a-ii](phase-5d-2-asbuilt.md#row-37), which also records it as an **open follow-up**: the fix belongs in the CHF layer, not a per-route workaround, but it would move `/json/`, `/oauth2/` and `/uma/`, each with its own byte oracle |
