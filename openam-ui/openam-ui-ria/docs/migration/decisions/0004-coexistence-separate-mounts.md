...
- **Build requirement (consequence):** the new app must be **path-relocatable** — the *same* build serves from `/EUI` during coexistence and `/XUI` after, with no rebuild. Vite `base` relative/injected at serve time (not hardcoded `/EUI`); react-router `basename` from runtime config; all internal links relative; no absolute `/EUI/...` strings.
