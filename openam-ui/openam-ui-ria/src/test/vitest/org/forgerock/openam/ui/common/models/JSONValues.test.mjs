...
/*
 * ES module port of src/test/js/org/forgerock/openam/ui/common/models/JSONValuesTest.js -- same
 * tests, same names, same order.
 *
 * NOT CHANGED, THOUGH IT ASSERTS NOTHING: in "returns an JSON string",
 * `expect(JSON.parse(values)).to.not.throw` reads `.throw` as a PROPERTY rather than calling it,
 * so that line is inert -- and it would be inert under chai 3 and Karma too. Transcribed as
 * found, because fixing it here would be a rewrite and would change what the case tests. The
 * neighbouring `expect(values).to.be.a("string")` is real and does carry the case. Flagged so
 * that whoever tightens this suite knows it is a known gap rather than an oversight.
 */
