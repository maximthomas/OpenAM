 ...
 * 2.9 adds `serverinfo/*`, which is the site configuration, and the footer's `serverinfo/version`;
 * 2.10 adds the realm writes, the one-call header authentication the fixtures provision through,
 ...
 * `roles` a profile read carries are what get the default route off `#login`, and which route it
 * picks [observed: xui/xui-realms.spec.mjs and xui/xui-services.spec.mjs, 7 passed each, and
 * xui/xui-profile.spec.mjs against this server]. What stops it next is whatever a spec beyond those
 * reaches for.
 *
...
8080. Free it, or use --port / OPENAM_LOCAL_PORT.`)
  ...
  REST  ${origin}/${options.context}/json/   (${state.realms.size} realm(s) from the capture; 501
                                              ...
                                              the bootstrap's configuration, realm and
                                              service admin, and the user profile)
