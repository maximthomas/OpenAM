...
| 5b-2 | device/user, checkSession (**realm-prefixed only — JSP kept**), endSession. **Split 3-way** — **5-E3** (gate: ~14 live-Restlet contract rows; none of the three is locked today) → **5b-2a** `EndSessionHandler` + `CheckSessionHandler` → **5b-2b** `DeviceCodeVerificationHandler`. Detailed plan: [phase-5b-2.md](phase-5b-2.md) | **5-E3 done** 2026-07-28 (test-only, 14 e2e rows, `npx playwright test oauth2` **62 passed**, was 48); **5b-2a done** 2026-07-28 — `f1ffda5d28` (D10 alone) + `67c71eb41e` (the handler pair, checklist steps 6–10): `EndSessionHandler` + `CheckSessionHandler` on `AbstractOAuth2HttpJsonEndpoint` unchanged, **1157 surefire + 18 failsafe** (was 1132 after 5b-1), e2e `oauth2` **63 passed**, grep gate 0, routed nowhere until 5d-1. Review found a wire-visible plan gap — [D7](phase-5b-2.md#d7)'s wrap list missed that `OpenIDConnectEndSession.endSession` reconstructs the id_token *itself*, so a malformed `id_token_hint` was a framework **500** against live Restlet's 400 `server_error`, and the first test for it was a **false green** off an unstubbed mock. Two divergences found and recorded rather than fixed ([rows 9 and 10](#expected-divergences-at-the-flip)), both applying beyond this step ([as-built](phase-5b-2.md#as-built-5b-2a--landed-2026-07-28)); **5b-2b done** 2026-07-28 — `06f9251ce6`: `DeviceCodeVerificationHandler` on `AbstractOAuth2HttpBrowserEndpoint` + the [D2](phase-5b-2.md#d2) `ConsentPageRenderer` correction (phase 1 copies all nine `MODEL_KEYS` from the attributes, not `realm` alone — the device consent model has **no query to come from**), **1180 surefire + 25 failsafe**, grep gate 0, doclint clean, routed nowhere until 5d-1. The plan held: [D2](phase-5b-2.md#d2)/[D3](phase-5b-2.md#d3)/[D4](phase-5b-2.md#d4) went in as written and [finding 7](phase-5b-2.md#7-the-device-verify-control-flow-branch-by-branch) called both traps — the `InvalidGrantException` catch must stay scoped to `readDeviceCode` alone (`update`/`deleteDeviceCode` throw the same type), and the IAE divergence belongs in **row 1**, not a new row. ⚠ **The false green recurred**: three "unusable user code" rows were green off an unstubbed `user_code` (`anyString()` does not match `null`), so all three silently took the `deviceCode == null` branch and the `InvalidGrantException`/`isIssued` paths were never executed — the same shape as 5b-2a's unstubbed mock, again caught by *reviewing* the tests. `DeviceCodeRouteCompositionIT` drives the **real** `OAuth2RequestFactory` in every row (the checklist asked for one), which is the only way the request cache is testable at all ([finding 14 / R-5b2.3](phase-5b-2.md#14--the-request-cache-is-load-bearing-here-and-nothing-tests-it)); mutation-checked — removing the one seeding call turns 5 of its 7 rows red ([as-built](phase-5b-2.md#as-built-5b-2b--landed-2026-07-28)). Two corrections to the umbrella, both verified: **(1)** only **one** of the three is a browser endpoint — the `doCatch` arity is the error contract, and `EndSession`/`OpenIDConnectCheckSessionEndpoint` call the **2-arg** `ExceptionHandler.handle` (**JSON**), so they extend `AbstractOAuth2HttpJsonEndpoint`, not the browser base ([finding 1](phase-5b-2.md#1--only-one-of-the-three-is-a-browser-endpoint-the-doccatch-arity-decides)); **(2)** `ConsentPageRenderer`'s phase 1 (shipped in 5b-1) is **realm-only**, and the device flow seeds its whole consent model from request *attributes* — so as it stands the device consent page silently loses `client_id`/`scope`/`state`/`nonce`/`response_type`/`ui_locales`/`realm` ([finding 2](phase-5b-2.md#2--consentpagerenderer-phase-1-is-realm-only-and-that-silently-breaks-the-device-consent-page), fixed by D2). **Reviewed 2026-07-28**, which caught a defect in the plan's own first draft: a proposed `RuntimeException` mapper on the shared JSON base re-litigated locked **D3** (bug paths keep CHF's 500) — replaced by narrow, source-level `ServerException` wraps for the three *client-reachable* unchecked throws, touching no shared class ([D7](phase-5b-2.md#d7)). Review also added **[D10](phase-5b-2.md#d10)**: `OAuth2ErrorFilter.errorFor` gains `case 405 → method_not_allowed`, one line in our own code that **narrows expected-divergence #3** to `error_description` only for `/authorize` and `/access_token` (the two `OAuth2Filter`-wrapped routes). **No CHF/openam-http change is needed** — `AnnotatedMethod` already resolves the most-specific `@ExceptionHandler` on inherited methods. **[5-E3 as-built](phase-5b-2.md#as-built-5-e3--recorded-2026-07-28)** settled the three gated decisions: **D5** stands (every non-`page` `?display=` is a 400, by three different mechanisms); **D8** resolved — `Reference` does **no** normalisation, an existing query survives verbatim and `state` appends with `&`, encoded `%20`/`%2F`; **D7** confirmed **and extended to a fourth wrap** — `getClientSessionURI` throws `NoSuchElementException` whenever a client's `clientSessionURI` is unset, which is the admin API's default, so check-session 400s on its own happy path today. **D10's justification narrowed**: none of the three emits `method_not_allowed` (all three send a CREST 405 body), so D10 rests on `/authorize` + `/access_token` alone |
| 5c | resource_set — handler + CHF `ResourceSetRegistrationExceptionFilter`, guarded by `ChfAccessTokenProtectionFilter(null)`. **Split 3-way** — **5-E4** (gate: 17 items / 20 live-Restlet rows; the existing 8 e2e rows are behaviour coverage, not a byte lock) → **5c-1** error-shape substrate → **5c-2** the handler. Detailed plan: [phase-5c.md](phase-5c.md) | **5-E4 done** 2026-07-29 (test-only, one new describe, 20 rows; `npx playwright test oauth2 uma` in one pass on a freshly built container = **94 passed** — oauth2 **83**, was 63, and `uma` **11**, unchanged; zero main-source lines) — [as-built](phase-5c.md#as-built-5-e4--recorded-2026-07-29). **Planned** 2026-07-29. The umbrella's two-line sizing is wrong in two load-bearing ways, both verified: **(1)** `resource_set`'s 401 is **OAuth2-shaped** (`{"error":"invalid_token"}`, pinned live at `oauth2-endpoints-test.spec.mjs:130-149`), not the CREST shape `ChfAccessTokenProtectionFilter` emits — its `ResourceSetRegistrationExceptionFilter` sits **outside** the protection filter and fills the entity before the app `StatusFilter` ever sees it, so [chf-patterns §16](chf-patterns.md#16-the-restlet-statusservice-renders-a-crest-body-for-bare-error-statuses-phase-4)'s CREST rule does **not** apply here. The 4a filter is live on `/uma` and cannot simply be reused ([finding 1](phase-5c.md#1--the-resource_set-401-is-oauth2-shaped-not-crest--the-4a-filter-cannot-be-reused-unchanged), [D2](phase-5c.md#d2)); **(2)** `resource_set` is the migration's **only** conditional-request endpoint, and Restlet's `ServerResource` did the `If-Match` **matching** for it — `isConditionalRequest()` only asks whether the header is present. `Endpoints.from` has no equivalent, so the literal port silently drops lost-update protection ([finding 2](phase-5c.md#2--restlets-conditional-request-machinery-is-load-bearing-and-chf-has-none), [D6](phase-5c.md#d6); facts disassembled from the fork, [chf-patterns §21](chf-patterns.md#21-restlets-conditional-request-machinery-phase-5c)). Also recorded: the `512` throw reaches the wire as **400 `server_error`** and never as 412 ([finding 4](phase-5c.md#4--the-512-throw-never-reaches-the-wire-as-512)); `org.forgerock.oauth2.restlet.resources` is **not** wholly deletable at 5d-2 — two of its three classes are Restlet-free and used by openam-uma ([finding 8](phase-5c.md#8--the-restlet-resources-package-is-not-deletable-at-5d-2)); and this is the first CHF route where the audit filter and the handler both read the **request** body (risk #1's first real occurrence). **Reviewed 2026-07-29**, which found two defects in the plan's own first draft: **(1)** [risk #8](#risk-register-behavioral-compatibility)'s *"register all three CHF routes; trailing-slash is a distinct CHF route"* is **not implementable** — commons `UriTemplateParser.createRegex` strips a template's trailing slash while `Paths.getPathElements` preserves the request's, so `EQUALS "resource_set/"` matches **nothing** and the obvious transcription 404s `/oauth2/resource_set/`, a URL the e2e list row and the UMA UI both use ([finding 12](phase-5c.md#12--the-trailing-slash-route-cannot-be-expressed-with-equals-in-chf)); the fix is a **nested** router (`STARTS_WITH "resource_set"` + child `EQUALS ""` / `EQUALS "{rsid}"`), which also moved the error filter onto the handler rather than the child router — wrapping the router would have turned a router 404 into a 500 ([D9](phase-5c.md#d9), [chf-patterns §22](chf-patterns.md#22-chf-uri-template-matching--trailing-slashes-variables-and-head-phase-5c-review)); **(2)** **`HEAD` regresses from 200 to 405 on every endpoint this migration has ported** — Restlet rewrites `HEAD` → `GET` before annotation lookup, `Endpoints.from` maps only `{DELETE, GET, POST, PUT}`. Not a 5c problem and **not in the divergence table**; the fix is two lines and the decision is owed at **5d-1** for all 15 endpoints at once ([finding 13](phase-5c.md#13--head-is-served-by-restlet-and-405d-by-chf--and-it-is-not-a-5c-problem), C3 in [decisions.md](decisions.md#chf-cleanup-backlog)). **A third pass** then found that producer B takes the wire status from `OAuth2Exception.getStatusCode()`, **not** from the status the protection filter set — so a token-store `ServerException`, reported as a **500**, reaches a `resource_set` client as a **400** (`ServerException` is `super(400, …)`, [chf-patterns §17](chf-patterns.md#17-oauth2exception--http-status-quirks-phase-5a-2b)), while the same exception on `/uma` stays a 500. [D2](phase-5c.md#d2) must derive status *and* error from the exception in OAuth2 mode and keep the passed code in CREST mode ([finding 14](phase-5c.md#14--the-protection-filters-chosen-status-is-overridden-by-the-exceptions-and-a-500-becomes-a-400)). **Fourth and fifth passes** went at the plan's own verification rather than the port: composition IT row 2 could not have caught the mutation hazard it was assigned to (audit builds its event **before** `next.handle`, and `Entity.getJson()` memoises, so risk #1 has no JSON failure mode — [finding 7](phase-5c.md#7--audit-matrix-and-the-body-contention-it-creates)); the IT chain as specified carried no `RealmContext`, so it would have certified a chain whose create path NPEs on `realm.asPath()` ([finding 16](phase-5c.md#16--the-create-path-needs-the-realm-object-and-the-phase-4-urisfactory-trap-does-not-recur) — the `OAuth2UrisFactory` Phase-4 trap itself does **not** recur, verified); and the conditional `If-Match` tag is `readResourceSet`'s, computed over a description whose `labels` come from **`umaLabelsStore`**, so hashing the bare `store.read` result 412s **every labelled resource set** while passing every test written with an unlabelled one ([finding 17](phase-5c.md#17--the-conditional-comparison-tag-comes-from-the-read-model-labels-and-all), R-5c.11). **5-E4 then recorded the live contract and corrected the plan in six places**, every one of them wire-visible ([as-built](phase-5c.md#as-built-5-e4--recorded-2026-07-29)): **(1)** ⚠ **`PATCH` is a working full update on this endpoint, not a 405** — Restlet routes it to the `@Put` method and it really replaces the resource set, while `Endpoints.from` maps only `{DELETE, GET, POST, PUT}`, so the port turns a working update into a 405; the 405 *shape* the row was written for was recorded off `OPTIONS`/`PROPFIND` instead (**405 `{"error":"unsupported_method_type"}` + `Allow: POST, PUT, GET, DELETE`** — a **third** 405 shape under `/oauth2`, alongside D10's `method_not_allowed` and the CREST one of [expected-divergence row 8](#expected-divergences-at-the-flip) — whose route list this finding also **corrects**, since it wrongly included `resource_set`); **(2)** the 400's `error_description` carries the Restlet exception's **formatted** message — `precondition_failed (512) - Require If-Match header to update Resource Set` — so [D4](phase-5c.md#d4)'s proposed `ServerException("Require If-Match header to update Resource Set")` would drop the `precondition_failed (512) - ` prefix; the general rule is that **every non-`OAuth2Exception` throwable here reaches the wire as 400 `server_error` with `error_description` = the Restlet `ResourceException`'s formatted message**; **(3)** the **304 carries the `ETag`** (the plan predicted it would not) and sends no `Content-Type`, and `If-None-Match: *` answers **200** where RFC 7232 §3.2 asks for 304; **(4)** the collection with `If-Match: *` is a 500-derived **400**, not the predicted 404 — though the reasoning that `*` passes the conditional layer and a concrete tag 412s first held exactly; **(5)** **media types are not enforced at all** (`text/plain`, `application/xml` and an absent `Content-Type` all 201), so [finding 3](phase-5c.md#3--three-error-producers-one-endpoint--and-only-one-of-them-is-the-endpoint)'s predicted NPE never fires, [D7](phase-5c.md#d7) resolves to *accept, no divergence*, and C1's `@Consumes` deletion costs no parity; **(6)** row 17's CREST 404 is right but is raised by **realm resolution above the OAuth2 router** and **before authentication** (a bogus bearer changes nothing), not by Restlet's router. ⚠ **[Finding 17](phase-5c.md#17--the-conditional-comparison-tag-comes-from-the-read-model-labels-and-all) is now demonstrated rather than argued**: whenever the client's `labels` order differs from the one the label store reads them back in, the ETag the `POST` returned never matched anything and the client's next conditional `PUT` **412s**, while the byte-identical request in the store's own order succeeds — row 2b pins both halves, which is R-5c.11's mutation check available before a line of the handler exists. ⚠ **A first draft of the as-built called that order *sorted*; it is not.** `UmaLabelsStore.query` returns a `HashSet<ResourceSetLabel>` and `ResourceSetLabel.hashCode()` mixes in `LabelType`'s **enum identity hash**, so the iteration order is fixed within a JVM and can flip on restart — verified by restarting the container and watching the same two labels read back reversed. Two consequences: a row that hard-codes an order is a coin flip per restart (row 2b discovers it instead, row 5 uses an unlabelled set), and **a restart silently invalidates every cached ETag for a resource set with ≥ 2 labels**, since `List.hashCode` is order-sensitive. A port that implements *sorting* would emit tags Restlet never emitted ([as-built](phase-5c.md#the-label-order-is-not-sorted)). The gate also produced the **`If-Match` parsing table** [D6](phase-5c.md#d6)'s helper must satisfy, including the rule the plan did not know: an **unparseable** `If-Match` (unquoted, empty, garbage) is treated as **absent** and takes the 400 path, not the 412 one. **Four** items handed to **5d-1**: `HEAD` (200 today, 405 after the flip, on all 15 endpoints), the row-17 pre-auth 404, `PATCH`, and — verified in `openam-http` — the **`Allow` header vanishing from every 405**, since `Endpoints.from`'s unmapped-verb branch sets no `Allow` at all where Restlet sends `Allow: POST, PUT, GET, DELETE`. RFC 7231 §6.5.5 makes it mandatory on a 405, it is Phase-5-wide like `HEAD`, and because [D3](phase-5c.md#d3) keeps the 405 *body* identical a byte-diff of the body at the flip will show nothing — the header is the entire divergence. **5c-1 done** 2026-07-29 — `fe0820050e`, the three collaborators (`HttpConditions`, `ResourceSetErrorFilter`, `ChfAccessTokenProtectionFilter`'s `ErrorShape` overload); **1226 surefire** + `openam-uma` **196**, and on a rebuilt container `oauth2 uma` = **94** with the CREST row `uma-test.spec.mjs:152` unedited — the load-bearing proof that [D2](phase-5c.md#d2) left `/uma` alone. ⚠ Its review found the step's sharpest lesson: S1 had disassembled the `If-Match` *parser* and never opened the **comparator**, so 21 green rows hid two reintroductions of the very lost-update hole [D6](phase-5c.md#d6) exists to close (`If-Match: W/"stale", *` and `W/*` both applied the update; `If-None-Match` compared weakness where `If-Match` does not). Neither was reachable by any 5-E4 row, so the oracle would not have caught them either — they were found by reading the second half of the bytecode. Fixed to Restlet's own two-field `Tag`; **27 rows**, 1233 surefire. **5c-2 done** 2026-07-30 — `cfc213ae11`, `ResourceSetRegistrationHandler` (537 lines, business logic ported verbatim) + 36 unit rows + the 13-row `ResourceSetRouteCompositionIT`; **1274 surefire + 38 failsafe**, doclint clean, import gate **0**, route gate clean, and `oauth2 uma` = **99 passed** in one pass on `openam-e2e:5c2`, built from this tree and verified by checksumming the deployed jar rather than trusting the banner ([as built](phase-5c.md#as-built-s7)). Routed **nowhere** until 5d-1. Two rows of the composition IT had to depart from the plan because writing them as specified would have produced a **green test asserting the wrong thing**: `PATCH` is not an unmapped verb (correction 1), so a plan-faithful 405 row would have passed while locking a divergence in as parity — the row uses `PROPFIND`; and replaying the `GET`'s ETag as `If-Match` is self-consistent under exactly the bare-`store.read` mutation the row exists to catch, so the row **computes** the expected tag instead. Four post-migration tickets raised ([T5–T8](#post-migration-tickets--raised-by-the-port-deliberately-not-fixed-in-it)) and **two** expected divergences of its own ([rows 12 and 13](#expected-divergences-at-the-flip)). ⚠ **Reviewing the finished change found four things all seven gates had passed over** ([review](phase-5c.md#s7-review-findings)), one of them not a 5c problem at all: **every charset-bearing `Content-Type` gains a space after the `;` on CHF** — commons `ContentTypeHeader.getValues()` renders `"; charset="` unconditionally, so it cannot be fixed in a handler, and it is **already shipped** in the 5b-1/5b-2b browser endpoints where `FreemarkerTemplateRenderer` and nine-plus committed rows pin the spaced form while 5-E2 row 2 recorded Restlet's un-spaced one. Now Phase-5-wide [row 11](#expected-divergences-at-the-flip). The other three: response-body key order (row 13); [D3](phase-5c.md#d3)'s ordering diagram contradicting [D9](phase-5c.md#d9) on whether audit or the error filter is outer — which decides whether the audit record holds the body the client received; and T8 instructing its own implementer to delete an `org.json` dependency five live classes still import |
| 5d-1 | **the flip, Restlet dormant** — `OAuth2HttpRouteProvider` (18 attachments + audit matrix) + services + web.xml mapping move + hook re-sign; **Restlet stack left in place** (one-line revert); soak green. ⚠ **Re-run the §E/§E2 e2e specs and byte-diff; the differences below are expected and must not be "fixed"** — see [expected divergences at the flip](#expected-divergences-at-the-flip). ⚠ **Five items 5c records but cannot close, all owed here** — the authoritative list is the [5c as-built's *Handed to 5d-1*](phase-5c.md#handed-to-5d-1), because that is what the flip reads: **(1)** `resource_set` must be mounted as a **nested** router (`STARTS_WITH "resource_set"` + child `EQUALS ""` / `EQUALS "{rsid}"`) — three `EQUALS` siblings **cannot** express the trailing-slash route and 404 `/oauth2/resource_set/` ([D9](phase-5c.md#d9)); `OAuth2RouterIT` must drive the **real** provider over all three URL forms, as `UmaRouterIT` does, because 5c's composition IT wires the shape inline and so cannot prove the provider uses it (R-5c.12); **(2)** decide **`HEAD`** once for all 15 endpoints — Restlet answers 200-no-body, `Endpoints.from` answers **405**; fix in two lines or add a divergence row (C3 in [decisions.md](decisions.md#chf-cleanup-backlog)); **(3)** 5-E4 rows 15/17 are recorded but deliberately unasserted on the CHF side until this step; **(4)** **`PATCH`** — Restlet routes it to the `@Put` method and it really updates, `Endpoints.from` 405s it. Same shape as `HEAD` but **not** Phase-5-wide: it matters wherever a `@Put` exists, which on the ported surface is this endpoint alone; **(5)** the **`Allow` header vanishes from every 405** — `Endpoints.from`'s unmapped-verb branch sets none, Restlet sends `Allow: POST, PUT, GET, DELETE`. RFC 7231 §6.5.5 makes it mandatory, so this is a spec regression and it is Phase-5-wide like `HEAD`. ⚠ [D3](phase-5c.md#d3) keeps the 405 *body* identical, so **a body byte-diff at the flip shows nothing** — the header is the entire divergence. Fix (two lines, beside the `HEAD` one) or add a divergence row. **Detailed plan: [phase-5d-1.md](phase-5d-1.md)** (2026-07-30) — it splits the step **four ways** (**5-E5** the last live-Restlet gate → **5d-1a** the `openam-http` verb fixes → **5d-1b** the provider + `OAuth2RouterIT`, still unflipped → **5d-1c** the one web.xml line) and answers all five hand-downs: `HEAD` and the missing `Allow` are **fixed** in `openam-http` (own commit, F5 — `Allow` deliberately excludes `HEAD` so 5-E4 row 11's recorded set still matches), `PATCH` is **recorded** as divergence row 14 (aliasing it to `PUT` would impose RFC 5789-wrong full-replace `PATCH` on every CHF endpoint with a `@Put`), the nested router + R-5c.12 land in 5d-1b, and rows 15/17 are asserted CHF-side in 5d-1c. Four corrections to this row, all verified: **(a)** the **hook re-sign is already done** — `LoginHintHook` implements all four interfaces and `OAuth2GuiceModule:236-242` binds both CHF Multibinders, so 5d-1 does no hook work; **(b)** registering the provider in `META-INF/services` is **itself a production change before any flip** — `Endpoints.from(Class)` resolves all 15 handlers when `HttpRouterProvider` builds the **single** router that also serves `/json`, so a bad binding breaks the console rather than `/oauth2`, which is why 5d-1b is its own commit soaked with the full e2e suite; **(c)** an unrouted `/oauth2` path is a **bodiless** 404 on CHF (commons `Router.newNotFound()`) and the realm filter breaks out rather than 404ing, so row 17 has no counterpart at all — a default-route `OAuth2NotFoundHandler` answers `{"error":"not_found"}` instead (divergence row 15); **(d)** **the trailing-slash question was never asked of the other 14 endpoints** — `resource_set` needed a nested router because CHF `EQUALS` cannot match a path ending in `/`, and disassembling the fork shows Restlet's `Router.attach` derives its mode from the **target** (`getMatchingMode`: `STARTS_WITH` for a `Router`/`Directory`, recursing through `Filter.getNext()`, else the `MODE_EQUALS` default) — every `/oauth2` endpoint row bottoms out at a `Finder`, so those are whole-string `EQUALS` and a flat table *should* be right, while `/realms/{realmId}` targets a router and is `STARTS_WITH`, which is what makes the realm-prefixed URLs work. ⚠ The first review pass got this wrong by reading only the default mode; 5-E5 row 13 measures it first because a 200 there would redesign the provider; **(e)** **risk #9 (realm styles) has never been recorded live on `/oauth2`** — e2e uses `/realms/root/` on two endpoints and `?realm=`/legacy path realms nowhere — which is why a final **5-E5** gate is added rather than flipping on the existing 99 rows. **A second review pass (2026-07-30) added three more, from reading the *Restlet* side rather than the CHF side:** **(f)** ⚠ **the flip adds host validation `/oauth2` has never had** — `RealmContextFilter:229-231` rejects any `Host` outside the configured FQDN map with a **400**, and on the `realms/` branch `HostnameFilter` 400s a host that is not a realm alias, where Restlet resolves the host **only** on the non-`realms/` branch and does no FQDN test at all. So `/oauth2/realms/root/authorize` behind a proxy that does not rewrite `Host` **works today and 400s after the flip**; no e2e row can see it (the suite always uses the container's hostname), so it is 5-E5 row 14, risk R-5d1.9, provisional divergence row 17 and a release-note line; **(g)** **`?realm=<bogus>` changes status 404 → 400** — Restlet's `RestletRealmRouter` maps the lookup failure to `CLIENT_ERROR_NOT_FOUND`, CHF's `RealmContextFilter` to `BadRequestException` (divergence row 16 — **confirmed** by 5-E5 row 2, which also corrected its body) — while `/oauth2/realms/<bogus>/…` keeps its **404** on CHF but ⚠ **not its message**: Restlet renders the *cause's* text (`No mapping organization found for organization identifier: /bogus`), not the `Realm "bogus" not found` description its router wrote, because `RestStatusService:42-52` prefers `status.getThrowable().getMessage()` — so `errorFor`'s new `case 404` buys vocabulary consistency, not parity ([5-E5 correction 3](phase-5d-1.md#5-e5-correction-3)); and **(h)** why Restlet's routing 404s come from **two** producers: `OAuth2RouterProvider:95` makes the whole `/oauth2` router a `RestletRealmRouter` whose default route is `/{subrealm}` in `MODE_STARTS_WITH`, so an unmatched element is looked up as a realm — but only when a **second** element follows it, since one element alone leaves the recursion with an empty URI that matches nothing and Restlet's own router 404 surfaces ([5-E5](phase-5d-1.md#5-e5-the-one-mechanism); the same attribute-ordering rule refutes the predictions in rows 4 and 14). ⚠ 5d-2 note: **two different classes are named `RestletRealmRouter`** (the deprecated openam-restlet one, deletable; `RealmRoutingFactory`'s private inner one, which must survive). The step also **edits two committed pins deliberately** (`OAuth2ErrorFilterTest`'s 404 row and `ResourceSetRouteCompositionIT` row 9), because giving the router 404 a body changes both — the second one's counterfactual must be re-expressed, not deleted, or [D2](phase-5c.md#d9)'s only executable guard lapses | **5-E5 done** 2026-08-04 (test-only, one new describe, **14 rows**; `npx playwright test oauth2 uma` in one pass on a freshly built `openam-e2e:5e5` = **113 passed** — oauth2 **102**, was 88, and `uma` **11**, unchanged; zero main-source lines) — [as-built](phase-5d-1.md#as-built-5-e5--recorded-2026-08-04). The last live-Restlet oracle is now recorded, so 5d-1a/b/c may proceed. ⚠ **The gate corrected the plan in six places, five of them wire-visible**: **(1)** a flat `/oauth2/realms/<non-root>/…` is **served** by Restlet, in that sub-realm — the predicted CHF-only capability gain does not exist; **(2)** ⚠ **`HEAD /oauth2/authorize` is a 405 today**, not a run of the flow ([finding 6](phase-5d-1.md#6--head-after-the-fix-lands-on-code-paths-that-are-already-correct) was wrong: the endpoint's method filter sits *above* Restlet's `HEAD`→`GET` rewrite), so 5d-1a's `Endpoints.from` fix must be paired with an explicit `HEAD` refusal in `AuthorizeHandler` or a `HEAD` would start **issuing authorization codes**; **(3)** the realm 404's message is the *cause's*, so [D5](phase-5d-1.md#d5)'s byte-parity claim is struck; **(4)** `PATCH` runs the resource's `@Get` **first** and only then 405s — so it is wider than `resource_set` (`PATCH /oauth2/tokeninfo` without a token is a **401**), and divergence row 14 is widened; **(5)** [D11](phase-5d-1.md#d11) settles — Restlet **honours** `X-HTTP-Method-Override`, so the POST case is parity, but it honours it on **any** verb and also honours the `?method=` query tunnel on a POST, neither of which CHF implements: two new divergences that each turn a 405 into a 200, one of them **issuing a token**; **(6)** ⚠ an unknown request `Host` **500s on both URL styles today** — [finding 16](phase-5d-1.md#16--the-flip-adds-host-validation-to-oauth2-that-restlet-never-did)'s "works today, 400 after the flip" is refuted, so divergence row 17 becomes 500 → 400 and R-5d1.9 is downgraded from the step's most dangerous row to a release-note line. Confirmed as predicted: **[D1](phase-5d-1.md#d1)'s flat `EQUALS` table** (row 13, run first because a trailing-slash 200 would have forced [D2](phase-5d-1.md#d2)'s nested shape on all 15 endpoints — every form is a 404, R-5d1.8 discharged), case-sensitivity ([risk #11](#risk-register-behavioral-compatibility) spot-check), and divergence row 16's **status**. Four further divergence rows (18–20 + the widened 14) are drafted in the as-built and land with 5d-1c. **Planned** 2026-07-30 |
...
larger one — it is Phase-5-wide and already in committed code ([review](phase-5c.md#s7-review-findings)).
...
match Restlet rather than recorded ([S1 review](phase-5c.md#-corrected-in-review-same-day--two-divergences-both-from-reading-the-parser-and-not-the-comparator)).
...
| 11 | Case sensitivity | Both Restlet templates and CHF matchers are case-sensitive | Spot-check `/oauth2/Authorize` → 404 pre & post. ✅ **Pre-flip half done** — [5-E5 row 13](phase-5d-1.md#the-recorded-rows): `/oauth2/Tokeninfo`, `/oauth2/TOKENINFO`, `/oauth2/Connect/jwk_uri` and `/oauth2/connect/JWK_URI` are all 404. Re-run the same row after 5d-1c for the post half |
