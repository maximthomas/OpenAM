...
describe('SessionExpired', () => {
  it('shows the session-expired title and clears the token', async () => {
  ...
  it('recalls whitelisted login params (not realm) in the return-to-login link', async () => {
    rememberLoginParams(new URLSearchParams('goto=/foo&realm=/bar&authIndexType=service'))

    renderSessionExpired()

    const link = await screen.findByRole('link', { name: 'Return to Login Page' })
    const href = link.getAttribute('href') ?? ''
    expect(href).toContain('goto=')
    expect(href).toContain('authIndexType=service')
    expect(href).not.toContain('realm')
  })
})
