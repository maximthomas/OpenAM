...
_Reference inventory, not a decision doc — see [`../decisions/`](../decisions/) for the ADRs behind these choices. Update this file whenever a slice adds/changes a `commons-ui-next` export or app wiring pattern; last updated for P1-5k (session-timeout dialog/monitor/guard — the login-parity gate), 2026-07-03._
...
- Note for P1-10 (hash-spelling normalization): `/failedLogin`, `/sessionExpired`, `/logout` were new EUI routes as of P1-5f, with `route-ownership.yml` still listing them `owner: xui` until the P1-5k parity-gate flip (now done, see below).
- `useSessionMonitor.ts` (P1-5k, `useSessionMonitor(onExpiry, intervalMs?)`) — background poll (default 30s, overridable) of `isSessionValid(amTransport, getToken())`; fires `onExpiry` once on the valid→expired transition (not on every tick while still expired), resets once the session is valid again (e.g. a fresh token from re-auth), pauses (no network calls) while there is no token, and clears its timer on unmount. Deliberately does **not** use `getTimeLeft` for detection: the shared mock's `DEMO_SESSION` fixture has fixed calendar timestamps that drift into the past as real time passes, so time-based expiry against it is not reproducible — `isSessionValid`'s token-identity check (`'username' in info`) is timeless. The `commons-ui-next/mock` fixture `EXPIRING_TOKEN_ID` is a token the mock does not recognize (401), used to drive expiry deterministically in tests.
- `isSelfServiceUser.ts` (P1-5k) — `isSelfServiceUser(sessionInfo?: AmSessionInfo): boolean`, always `false` today: `AmSessionInfo` carries no role field (role data would come from a separate user-profile fetch, not `/json/sessions`), so this stays on the legacy `else` branch (admin/unknown → modal) until real role wiring lands with the profile slice (**P2-4**). The `sessionInfo` param exists so callers don't need to change signature when that lands.
- `SessionTimeoutDialog.tsx` (P1-5k) — non-closable react-bootstrap `Modal` (`backdrop="static"`, `keyboard={false}`, no close button), title `common.form.sessionExpired`. Reuses `useAuthenticationFlow()` (fresh, no query string) + `CallbackForm` — the same login engine as the full-page flow, just hosted in a modal (legacy `RESTLoginDialog.js` re-rendering `RESTLoginView` inside a `BootstrapDialog`). Clears the stale client-side token on mount (legacy `SessionToken.remove()`) before re-authenticating. On `success`: `setToken(tokenId)` + `onResume()` — closes the dialog with **no navigation**, the user stays on the page they were on. On a non-408 `failure`: shows the generic auth-failed message and calls `restart()` (mirrors `LoginPage`'s failure handling); on a tracked-408 timeout: shows the timeout message via `isTimedOut` without restarting; on `startFailed` (the initial call itself throwing): shows `openam.authentication.unavailable` in place rather than navigating to `/failedLogin` (a modal can't navigate away without abandoning the user's in-progress work). No remember-me integration (out of scope — the user is already identified mid-session) and `allowScriptExecution` stays at `CallbackForm`'s default `false`.
- `SessionGuard.tsx` (P1-5k) — layout route (`<Outlet>` + conditional dialog) mounted around the full-chrome `AppShell` route group in `App.tsx`. Runs `useSessionMonitor`; on expiry, `isSelfServiceUser()` → navigate `/sessionExpired`, else render `SessionTimeoutDialog`. Takes an optional `pollIntervalMs` prop (tests only; production relies on `useSessionMonitor`'s default) so tests don't have to wait out the real 30s interval.
- **Parity gate reached (P1-5k):** `route-ownership.yml` `login`/`confirmLogin`/`logout`/`failedLogin`/`sessionExpired` are now all `owner: eui, status: migrated`, mirrored in `routeOwnership.ts`. Login parity (P1-5) is complete.
...
- Real `isSelfServiceUser` role wiring (currently a hardcoded-`false` stub, see `features/auth` above) — **P2-4** (profile slice).
- Login parity (P1-5, all of P1-5b through P1-5k) is now complete — everything else that was tracked here has landed.
