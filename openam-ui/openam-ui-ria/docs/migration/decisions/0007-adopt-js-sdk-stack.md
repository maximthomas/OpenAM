...
Status: Accepted · Date: 2026-06-27 · Updated: 2026-06-27 (de-coupled from `openam-ui-js-sdk`) · Updated: 2026-07-02 (Vite version phrasing genericized; version-pinning consequence reworded to lockfile-based)
...
## Decision
Build the new app on **React 19, react-router 7, Vite (major version at scaffold time; `package.json` is authoritative), TypeScript 5.x (strict), Vitest, ESLint 9 (flat config), Sass** — current stable, widely-supported choices at scaffolding time. Scaffold fresh from the standard Vite React-TS template and configure it for this app's needs; do not copy another module's config.
...
- Versions are locked via the workspace lockfile; upgrade deliberately.
