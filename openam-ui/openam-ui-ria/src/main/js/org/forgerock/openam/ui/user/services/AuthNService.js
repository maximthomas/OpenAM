...
import {
    get as getAuthenticationToken, remove as removeAuthenticationToken, set as setAuthenticationToken
} from "org/forgerock/openam/ui/user/login/tokens/AuthenticationToken";
import {
    get as getSessionToken, isAuthenticated as isSessionAuthenticated, isResolvable,
    set as setSessionToken
...
function handleFragmentParameters (params) {
    ...
    if (sessionToken && isResolvable(sessionToken)) {
...
function addRealmToStore (realm) {
    store.dispatch(sessionAddInfo({ realm }));
}
...
obj.begin = function (options) {
    ...
    const urlAndParams = addQueryStringToUrl(
        fetchUrl("/authenticate", { realm: store.getState().server.realm }),
        urlParamsFromObject(handleFragmentParameters(parseParameters(fragmentParams)))
    );
    const serviceCall = {
        type: "POST",
...
obj.handleRequirements = function (requirements) {
    ...
    const isAuthenticated = isSessionAuthenticated(requirements);
    ...
    if (requirements.hasOwnProperty("authId")) {
        ...
        if (!getAuthenticationToken() && _.find(requirements.callbacks, callbackTracking)) {
            setAuthenticationToken(requirements.authId);
        }
    } else if (isAuthenticated) {
        setSessionToken(requirements.tokenId);
