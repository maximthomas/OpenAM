...
- `LoginPage.tsx` — the `/login` route. Drives `useAuthenticationFlow` (below), renders `CallbackForm` for `kind: 'requirements'` steps, intercepts `RedirectCallback` (builds+submits a hidden-field POST form or `location.replace` for GET), handles `success` (sets the token, resolves `goto` via `validateGoto` or navigates `/`) and the existing-session branch (`isExistingSession`: same realm → treat as success, different realm → navigate to `/confirmLogin`), and zero-page auto-login (pre-fills callbacks from `IDToken1..N` URL params and auto-submits when `isZeroPageLoginAllowed(serverInfo.zeroPageLogin, document.referrer)` — see the serverinfo note above).
