...
| 3 | Unsupported verb (`PUT`) body | 405 `method_not_allowed` | 405 `invalid_request` | [D8](phase-5b-1.md#d8) — the framework's 405, body rewritten by `OAuth2ErrorFilter`. **Status and cache headers now match**: `OAuth2NoCacheFilter` restored the `no-store`/`Pragma` that per-method stamping could not reach |
