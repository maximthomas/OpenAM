...
public class AMSignatureProvider implements SignatureProvider {
	...
	if (assertionID == null) {
        ...
        try {
            ElementProxy.setDefaultPrefix(Constants.SignatureSpecNS, SAMLConstants.PREFIX_DS);
            Element wsucontext = com.sun.identity.common.xml.XMLUtils.createDSctx(doc, "wsu", wsuNS);
	...
	try {
	    ...
	    Element reference = (Element) keyinfo.getElementsByTagNameNS(
	        ...
	        id = id.substring(1);
	        nscontext = com.sun.identity.common.xml.XMLUtils.createDSctx(doc, SAMLConstants.PREFIX_WSU, wsuNS);
