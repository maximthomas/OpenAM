...
public abstract class IDPPBaseContainer implements IDPPContainer {
     ...
     protected Map getAttributeMap(String attr, Object obj, Map map) {
        ...
        if(obj != null) {
           ...
           if(obj instanceof DSTString) {
              ...
              value = uri.getValue();
           } else if (obj instanceof DOBElement) {
               DOBElement date = (DOBElement)obj;
              XMLGregorianCalendar cal = date.getValue();
              if(cal != null) {
                 value = DateFormat.getDateInstance().format(cal.toGregorianCalendar().getTime());
              }

           } else if (obj instanceof AgeElement) {
               AgeElement dstInt = (AgeElement)obj;
               value = dstInt.getValue().toString();

           } else if (obj instanceof BirthdayElement) {
               BirthdayElement dstMon = (BirthdayElement)obj;
               XMLGregorianCalendar cal = dstMon.getValue();
               if(cal != null) {
                   value = DateFormat.getDateInstance().format(cal.toGregorianCalendar().getTime());
               }
           }
      ...
      */
     protected DOBElement getDSTDate(String value) {
        if(value == null) {
           IDPPUtils.debug.message("IDPPBaseContainer:getDSTDate:null vals");
           return null;
        }
        try {
            DOBElement dstDate = IDPPUtils.getIDPPFactory().createDOBElement();
            Date date = 
      ...
      */
     protected BirthdayElement getDSTMonthDay(String value) {
        if(value == null) {
           IDPPUtils.debug.message("IDPPBaseContainer:getDSTMonthDay:nullvals");
           return null;
        }
        try {
            BirthdayElement dstMonthDay =
                 IDPPUtils.getIDPPFactory().createBirthdayElement();
            Date date =
                    DateFormat.getDateInstance(DateFormat.MEDIUM).parse(value);
            GregorianCalendar cal = new GregorianCalendar();
            cal.setTime(date);
            XMLGregorianCalendar xmlCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(cal);
            dstMonthDay.setValue(xmlCal);
      ...
      */
     protected AgeElement getDSTInteger(String value) {
        if(value == null) {
           IDPPUtils.debug.message("IDPPBaseContainer:getDSTInteger:null vals");
           return null;
        }
        try {
            AgeElement dstInteger =
                 IDPPUtils.getIDPPFactory().createAgeElement();
      ...
      */
     protected AnalyzedNameElement getAnalyzedName(Map userMap)
     throws IDPPException {
        IDPPUtils.debug.message("IDPPContainers:getAnalyzedName:Init");
        AnalyzedNameElement analyzedName = null;
         analyzedName = IDPPUtils.getIDPPFactory().createAnalyzedNameElement();
     ...
     protected Map getAnalyzedNameMap(Object obj, Map map) throws IDPPException{
        ...
        if(obj != null) { 
           if(obj instanceof AnalyzedNameElement) {
               AnalyzedNameElement analyzedName = (AnalyzedNameElement)obj;
