...
- **Neither 405 producer in `Endpoints.from` sent an `Allow` header**, which RFC 7231 §6.5.5 makes mandatory —
  ...
  5-E4 row 11's recorded set turns red for no behaviour gain. Recorded here rather than left implicit because
  the entry never existed as an open item — it went straight to closed.

- **`/json/api` 500s when the request carries no `Accept-Language` header.**
  ...
  dereferencing. **Status: deferred** — recorded rather than fixed inside a framework-verb commit whose blast
  radius must stay reviewable. Cheap to do in its own commit with a test.
...
- **`Endpoints.from` does not map `PATCH` at all — and Restlet routes it to `@Put`.** Same shape as the `HEAD`
  ...
  wherever a `@Put` exists, which on the ported surface is `/oauth2/resource_set` alone.
