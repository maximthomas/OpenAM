...
| 5a-1 | `/access_token` grant dispatcher (`TokenEndpointHandler`) + `AbstractOAuth2HttpJsonEndpoint` base + `TokenRequestHook` seam. **Cookie spike first** (settle servlet-response cookie survival before the hook seam). Detailed plan: [phase-5a-1.md](phase-5a-1.md) | pending |
