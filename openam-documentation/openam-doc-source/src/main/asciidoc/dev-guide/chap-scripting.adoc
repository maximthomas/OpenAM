...
* xref:#scripting-api-authn-session-data[Accessing User Session Data]
...
[#scripting-api-authn-session-data]
===== Accessing User Session Data
Server-side scripts can set user session properties using the `sessionProperties` object.

[#scripted-api-authn-session-data-methods]
.Authentication Script Session Data Methods
[cols="22%,33%,11%,34%"]
|===
|Method |Parameters |Return Type |Description

a|`sessionProperties.put`
a|`Property Name` (type: `String`)

`Property Value` (type: `String`)
a|`Void`
a|New session attribute value
|===
