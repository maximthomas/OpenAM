...
| 5b-1 | `AuthorizeHandler` **alone** (~600L) + `AbstractOAuth2HttpBrowserEndpoint` base (+ consent data model, catch-collapse, `AuthorizeRequestHook`). **Split 3-way** — **5-E2** (gate, above) → **5b-1a** browser substrate (`AbstractOAuth2HttpEndpoint` extract, browser `@ExceptionHandler` base, `ChfAuthorizeRequestHook`, the missing neutral `getAcceptedLanguages()`) → **5b-1b** `AuthorizeHandler` + shared `ConsentPageRenderer`. Detailed plan: [phase-5b-1.md](phase-5b-1.md) | planned 2026-07-25; **5-E2 done 2026-07-26** (S1–S2). Next: **S3**, the D1 `AbstractOAuth2HttpEndpoint` extract. 5b-1a/5b-1b pending |
