...
## What we're doing
Migrating `openam-ui-ria` (the OpenAM **XUI** SPA) off RequireJS/Backbone/Grunt onto **React 19 + Vite + TypeScript (strict) + react-router 7 + Vitest**, via an **incremental strangler-fig**. During coexistence the new app mounts at a **temporary `/EUI`**; legacy stays at `/XUI`; they run side by side until legacy is empty. **At cutover the new app is deployed to the canonical `/XUI`** (legacy deleted) — `/EUI` is temporary, `/XUI` is the final path (ADR-0004, ADR-0011). The new build must be **path-relocatable** (same build serves `/EUI` then `/XUI`).

## The two facts that drive everything
1. **The target stack is a deliberate modern baseline.** React 19 + Vite + TypeScript (strict) + react-router 7 + Vitest, chosen on its own merits (see ADR-0007). This app is self-contained — do **not** treat `openam-ui-js-sdk` (or any other module) as a template to copy; build the scaffold fresh.
...
## Status
See [`tasks.yml`](tasks.yml). Building a slice? Check [`reference/`](reference/) first — `eui-foundation.md` (new-app API surface) and `legacy-login.md` (legacy login feature map) — before re-deriving what already exists. **Phase 0 is complete** (P0-0 through P0-9 done): the `openam-ui/openam-ui-eui` Maven module, the `openam-ui/` npm workspace, the `eui` app scaffold, the `commons-ui-next` package, the Maven build wiring, the route-ownership map + `<CrossLink>` helper, CI lint/test, the `/XUI` URL audit, the shared MSW AM-REST handlers (`commons-ui-next/src/mock`), the EUI browser mock mode (`npm run dev:mock`), and the standalone mock server + legacy XUI no-AM harness (`npm run mock` / `npm run dev:xui`) are all in place. Phase 1 in progress — **login parity (P1-5, all of P1-5b–P1-5k) is done**: `route-ownership.yml`'s `login`/`confirmLogin`/`logout`/`failedLogin`/`sessionExpired` are all `owner: eui, status: migrated`. Remaining phase-1 work: P1-6 (anonymousProcess: password reset/self-register/forgot username, plus the login page's self-service links), P1-7/P1-8 (wire `/EUI` into the packaged war and deploy side by side), P1-10 (hash-spelling normalization map) — see `tasks.yml`.

## Watch out for
- **Final path is `/XUI`, not `/EUI`** — `/EUI` is only the coexistence mount. Never hardcode `/EUI` in the new app (Vite `base` relative/injected; no router `basename` — the app is `HashRouter`-routed, ADR-0011). Legacy `/XUI` deep links (hash-routed: `#login`, `#dashboard/`, password-reset/register regexes) keep resolving because the new app is also hash-routed; a hash-spelling normalization map (P1-10) bridges the `#login` → `#/login` leading-slash difference.
